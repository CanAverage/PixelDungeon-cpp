                    ...
                    float lineWidth)
{
	...
	glBegin(GL_LINE_STRIP);
	{
		...
		glVertex2f(centerX + radX * static_cast<float>(cos(tillAngle)),
		           centerY + radY * static_cast<float>(sin(tillAngle)));
	}
	glEnd();
}
...
bool utils::IsOverlapping(const Rectf& r, const Circlef& c)
{
	...
	if (DistPointLineSegment(c.center, {r.left + r.width, r.bottom + r.height}, {r.left, r.bottom + r.height}) <= c.
		radius) return true;
	if (DistPointLineSegment(c.center, {r.left + r.width, r.bottom + r.height}, {r.left + r.width, r.bottom}) <= c.
		radius) return true;
	return false;
}
